...
// One-off maintenance script: add three more predictions for "Mr. Boss Baby",
// each identified by its exact team pairing, without touching anyone else's data.
//
...
// matching seed-demo.mjs / add-boss-baby-predictions.mjs, we compute
// points_awarded here at insert time for finished matches (null if not finished).
//
//   node scripts/add-boss-baby-predictions-2.mjs            # dry run: report only
//   node scripts/add-boss-baby-predictions-2.mjs --apply    # actually write rows
//
// Picks ("team wins winGoals-loseGoals" over opponent):
//   England  win 2-1 over Mexico    (Mexico 1 - 2 England)
//   Norway   win 2-1 over Brazil    (Brazil 1 - 2 Norway)
//   France   win 3-1 over Paraguay  (Paraguay 1 - 3 France)
...
const PICKS = [
  { team: "england", opponent: "mexico", winGoals: 2, loseGoals: 1 },
  { team: "norway", opponent: "brazil", winGoals: 2, loseGoals: 1 },
  { team: "france", opponent: "paraguay", winGoals: 3, loseGoals: 1 },
];
...
async function main() {
  ...
  // 4) Resolve each pick to its unique fixture (matched on BOTH team names).
  const plans = [];
  for (const pick of PICKS) {
    ...
    if (fixtures.length !== 1) {
      throw new Error(
        `Expected exactly one fixture for ${pick.team} vs ${pick.opponent}, ` +
